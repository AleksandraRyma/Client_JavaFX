...
public class AdminDeleteUser {
    ...
    void deleteUser() throws ClassNotFoundException {
        Employee selectedEmployee = tableSeeUserAdmin.getSelectionModel().getSelectedItem();
        boolean confirmationResult = DialogAlert.showConfirmationDialog("Подтверждение", "Отмена", "Вы действительно хотите удалить эту запись?");
        if(confirmationResult){
            if (selectedEmployee != null) {

                Connect.client.sendMessage("adminDeleteUser");
                Connect.client.sendObject(selectedEmployee); // Отправляем ID на сервер
                Employee obj = (Employee) Connect.client.readObject();
                tableSeeUserAdmin.getItems().remove(selectedEmployee);
                DialogAlert.showAlertInfo("Пользователь с id " + obj.getEmployee_id() + " удалён");

            } else {
                DialogAlert.showAlertInfo("Пожалуйста, выберите пользователя для удаления.");
            }
        }
